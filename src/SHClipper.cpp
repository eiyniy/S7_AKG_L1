    ...
    const Point &edgeP1, const Point &edgeP2)
{
    ...
    for (int i = 0; i < polygonSize; ++i)
    {
        ...
        const auto point1 = Converter::vertexToPoint(vertex1);
        const auto point2 = Converter::vertexToPoint(vertex2);
        ...
        if (p1Rel < 0 && p2Rel < 0)
        {
            newVertexIds.emplace_back(VertexIds(polygon.cGetVertexIds(k).cGetVertexId()));
        }
