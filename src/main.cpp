...
int main(int argc, char **argv)
{
    ...
    // auto objInfoPt = parser.parseEntries(str);

    const auto videoMode = sf::VideoMode::getDesktopMode();
    Point cameraResolution{(int)videoMode.width, (int)videoMode.height};
    Camera camera{{0, 1, 0}, {-10, 100, 10}, {0, 0, 0}, cameraResolution, 100};
    Matrix<4, 1> lightPos{10, 100, 50};
    ...
    PhongModel lightingModel{0.05, 0.55, 0.4, 1};
    Scene scene{camera, &lightSource, 2, 2};
    MainWindow mainWindow{cameraResolution, &lightingModel, scene.cGetLightSource(), ShadingModel::Fong};
