...
template<int Rows, int Cols>
Matrix<Rows, Cols> &Matrix<Rows, Cols>::operator*=(const Matrix<Rows, Cols> &m)requires(Rows == Cols) {
...
template<int Rows, int Cols>
void Matrix<Rows, Cols>::normalize()requires(Rows == 4 && Cols == 1) {
    if (getLength() != 0.f)
        *this /= getLength();
    else
...
template<int Rows, int Cols>
Matrix<4, 4>
Matrix<Rows, Cols>::getViewConvert(const Matrix<4, 1> &eye, const Matrix<4, 1> &target, const Matrix<4, 1> &up) {
